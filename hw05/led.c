...
MODULE_PARM_DESC(gpioLED2, " GPIO LED 2 number (default=60)");

static unsigned int blinkPeriod = 1000;     ///< The blink period in ms
static unsigned int blinkPeriod2 = 600;
...
MODULE_PARM_DESC(blinkPeriod2, " LED blink period is 600ms");
...
static int __init ebbLED_init(void){
   ...
   gpio_export(gpioLED, false);  // causes gpio49 to appear in /sys/class/gpio
                                 // the second argument prevents the direction from being
   ...
   if(IS_ERR(task2)){
	   printk(KERN_ALERT "EBB LED: failed to create task2\n");
